  ...
  <j-modal
    ...
    <a-form layout="inline">
      <a-form-item label="客户" required>
        <a-select v-model="form.organId" placeholder="请选择客户" style="width: 300px;"
          showSearch :filterOption="filterOption" @search="handleSearchCustomer">
          <div slot="dropdownRender" slot-scope="menu">
            <v-nodes :vnodes="menu" />
            <a-divider style="margin: 4px 0;" />
            <div class="dropdown-btn" @mousedown="e => e.preventDefault()" @click="initCustomerList()">
    ...
    <template v-if="previewData">
      ...
      <a-row :gutter="16" style="margin-top: 16px;">
        <a-col :span="6">
          <a-card><a-statistic title="期初应收余额" :value="previewData.beginAmount" :precision="2" prefix="¥"/></a-card>
        </a-col>
        <a-col :span="6">
          <a-card><a-statistic title="本期发生应收" :value="previewData.periodAmount" :precision="2" prefix="¥"/></a-card>
        </a-col>
        <a-col :span="6">
          <a-card><a-statistic title="本期已收" :value="previewData.changeAmount" :precision="2" prefix="¥"/></a-card>
        </a-col>
        <a-col :span="6">
          <a-card><a-statistic title="期末应收余额" :value="previewData.endAmount" :precision="2" prefix="¥"/></a-card>
        </a-col>
      </a-row>
        ...
        <div style="margin-bottom: 12px; display: flex; justify-content: space-between; align-items: center;">
          <a-alert type="info" style="flex:1;">
            <template slot="description">
              当前共 <strong>{{ items.length }}</strong> 条出库明细，汇总金额会随明细变化自动更新
            </template>
          </a-alert>
...
<script>
  ...
  export default {
    name: 'CustomerReconciliationCreateModal',
    components: {
      VNodes
    },
    data() {
      return {
        ...
        detailColumns: [
          ...
          { title: '已收金额', dataIndex: 'changeAmount', width: 100, align: 'right', scopedSlots: { customRender: 'amountSlot' } },
          { title: '出库时间', dataIndex: 'operTime', width: 120,
    ...
    methods: {
      ...
      handleSave() {
        ...
        // 获取客户名称
        const cus = this.cusList.find(c => c.id === this.form.organId)
        const organName = cus ? cus.supplier : ''
      ...
      handleExportDetail() {
        try {
          const exportData = this.items.map((item, index) => ({
            ...
            '单价': item.unitPrice,
            '金额': item.allPrice,
            '已收金额': item.changeAmount,
            '出库时间': item.operTime ? item.operTime.substring(0, 10) : '',
            '备注': item.billRemark || ''
          }))
